...
def load_image_data(image_dir_path, image_height, image_width, labels, target_label_length):
    ...
    image_data = np.zeros(
        shape=(len(image_name_list), image_height, image_width, 1))
    label_data = np.zeros(
        shape=(len(image_name_list), len(labels) * target_label_length))

    for index, image_name in enumerate(image_name_list):
        img = keras_preprocessing.image.utils.load_img(
            os.path.join(image_dir_path, image_name), color_mode='grayscale')
...
class FixCaptchaLengthModel(object):
    ...
    def model(self):
        ...
        input = keras.Input(shape=(
            self.image_height, self.image_width, self.image_channel), batch_size=None)
        model.add(input)
        # 第一层 卷积
        ...
        model.add(keras.layers.Dropout(rate=self.dropout))
        # 第五层 全连接
        model.add(keras.layers.Dense(units=self.fixed_length *
                  self.label_number, activation="sigmoid"))
...
class CheckAccuracyCallback(keras.callbacks.Callback):
    ...
    def _compare_accuracy(self, data_x, data_y):
        ...
        predict_y = keras.backend.reshape(
            predict_y, [len(data_x), self.fixed_label_length, self.label_number])
        data_y = keras.backend.reshape(
            data_y, [len(data_y), self.fixed_label_length, self.label_number])
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        print('Epoch %s with image accuracy on train batch: %s' %
              (epoch, keras.backend.eval(on_train_batch_acc)))
        on_test_batch_acc = self._compare_accuracy(
...
class Predictor(object):
    ...
    def predict_remote_image(self, remote_image_url, headers=None, timeout=30, save_image_to_file=None):
        ...
        response = requests.get(
            remote_image_url, headers=headers, timeout=timeout, stream=True)
    ...
    def predict_single_image_content(self, image_content):
        ...
        image_data = np.zeros(
            shape=(1, self.config.image_height, self.config.image_width, 1))
        ...
        result = keras.backend.reshape(
            result, [1, self.config.fixed_length, self.label_number])
...
def train():
    ...
    callbacks = [
        keras.callbacks.ModelCheckpoint(
            filepath=config.model_save_path, save_weights_only=True, save_best_only=True),
...
if __name__ == '__main__':
    ...
    # for i in range(0, 30):
    #     count = 132 + i
    #     result = predictor.predict('./download_image/'+count+'.png')
    #     print(result)
    result = predictor.predict('./download_image/1.png')
    print(result)
    result = predictor.predict('./download_image/2.png')
    print(result)
    result = predictor.predict('./download_image/3.png')
    print(result)
    result = predictor.predict('./download_image/4.png')
    print(result)
    result = predictor.predict('./download_image/5.png')
    print(result)
    result = predictor.predict('./download_image/6.png')
    print(result)
    result = predictor.predict('./download_image/7.png')
    print(result)
    result = predictor.predict('./download_image/8.png')
    print(result)
    result = predictor.predict('./download_image/9.png')
    print(result)
    # result = predictor.predict('./download_image/1021.png')
    # print(result)
    # 直接二进制内容预测
